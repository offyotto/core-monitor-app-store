...
Mac system monitor with weather

## Description

Core Monitor gives you a readable live view of CPU activity, per-core load, memory pressure, network throughput, storage usage, uptime, battery status, system thermal state, and local weather when you allow location access.

It stays read-only. It does not install helper tools, control fans, change system behavior, or require an account.
